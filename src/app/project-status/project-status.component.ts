...
export class ProjectStatusComponent implements OnInit {
  ...
  processColors(data: IProjectStatus[]) {
    data.forEach(element => {
      ...
      let objColor: IMetricColors = {
        ...
        resourceRemarks: resData.remarks
      };
  ...
  colorChangeForScope(scopeValues: IMetricScope, release: string): any {
    ...
    if (scopeValues.developmentDependencies.toUpperCase() === 'Y' || scopeValues.developmentDependencies.toUpperCase() === 'YES') {
      ...
      Remarks.push(this.formRemarks('Dependencies identified partially / vague or no commitment on timelines' +
        'from the dependent team on delivery'));
    } else {
    ...
    if (scopeValues.tgoDesign.toUpperCase() === 'Y' || scopeValues.tgoDesign.toUpperCase() === 'YES'
      ...
      Remarks.push(this.formRemarks('TGO Design pending for Release #' + release + ' and #' +
        scopeValues.noOfDaysFromStartDate + ' days over from project started'));
    } else {
    ...
    if (scopeValues.tgoConstruction.toUpperCase() === 'Y' || scopeValues.tgoConstruction.toUpperCase() === 'YES'
      ...
      Remarks.push(this.formRemarks('TGO Construction pending for Release #' + release + ' and #' +
        scopeValues.noOfDaysFromCodeFreezeDate + ' more days left for code freeze'));
    } else {
